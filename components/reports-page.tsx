...
const allReportData = [
  {
    ...
    observacoes:
      "Paciente com melhora significativa na adesão após ajuste de horários",
  },
  {
    id: 2,
  ...
  },
  {
    ...
    observacoes:
      "Necessita de acompanhamento mais próximo, muitos esquecimentos",
  },
  {
    id: 5,
  ...
  },
  {
    ...
    horarioEsquecimento: ["Manhã", "Tarde", "Noite"],
    observacoes:
      "Muita dificuldade com múltiplos medicamentos, necessita assistência",
  },
];
...
const passesFilter = (item: any, filters: FilterValues): boolean => {
  ...
  if (filters.medicationType !== "all") {
    ...
    if (
      !medicationTypeMapping[
        filters.medicationType as keyof typeof medicationTypeMapping
      ]
    ) {
      return false;
    }
  ...
  if (filters.adherenceRate !== "all") {
    ...
    if (
      filters.adherenceRate === "medium" &&
      (item.adesao < 50 || item.adesao > 80)
    ) {
      return false;
    }
  ...
  if (filters.timeOfDay.length > 0) {
    ...
    const hasMatchingTime = filters.timeOfDay.some(
      (time) => timeMapping[time as keyof typeof timeMapping]
    );
...
export function ReportsPage() {
  ...
  const statistics = useMemo(() => {
    ...
    const adesaoMedia =
      filteredReportData.reduce((acc, item) => acc + item.adesao, 0) /
      filteredReportData.length;

    const alertasTotal = filteredReportData.reduce(
      (acc, item) => acc + item.alertasIgnorados,
      0
    );

    const medicamentosMedia =
      filteredReportData.reduce((acc, item) => acc + item.medicamentos, 0) /
      filteredReportData.length;

    const idadeMedia =
      filteredReportData.reduce((acc, item) => acc + item.idade, 0) /
      filteredReportData.length;

    const melhoriaMedia =
      filteredReportData.reduce(
        (acc, item) => acc + (item.adesao - item.adesaoAnterior),
        0
      ) / filteredReportData.length;

    const pacientesAltoRisco = filteredReportData.filter(
      (item) => item.adesao < 70 && item.medicamentos > 3
    ).length;

    const distribuicaoGenero = filteredReportData.reduce(
    ...
    const distribuicaoRegiao = filteredReportData.reduce(
      (acc: Record<string, number>, item) => {
  ...
  const exportToCSV = () => {
    ...
    link.setAttribute(
      "download",
      `relatorio_adesao_${new Date().toISOString().split("T")[0]}.csv`
    );
  ...
  const exportToPDF = async (type = "table") => {
    ...
    try {
      ...
      doc.text(
        "MedMonitor - Sistema de Monitoramento de Adesão",
        pageWidth / 2,
        10,
        { align: "center" }
      );
      ...
      const title =
        type === "table"
          ? "Relatório Detalhado de Adesão a Medicamentos"
          : "Resumo de Adesão a Medicamentos";
      ...
      if (
        ...
        if (filters.medicationType !== "all")
          filterText += `Tipo de Medicamento: ${filters.medicationType}, `;
        if (filters.adherenceRate !== "all")
          filterText += `Taxa de Adesão: ${filters.adherenceRate}, `;
        filterText += `Faixa Etária: ${filters.ageRange[0]}-${filters.ageRange[1]}, `;
        if (filters.timeOfDay.length > 0)
          filterText += `Horários: ${filters.timeOfDay.join(", ")}`;
      ...
      doc.text(
        "MedMonitor © 2025 - Todos os direitos reservados",
        pageWidth / 2,
        footerPosition,
        { align: "center" }
      );
      ...
      <div className="flex flex-col md:flex-row justify-between gap-4">
        <div>
          ...
          <p className="text-muted-foreground">
            Exporte e analise dados de adesão a medicamentos
          </p>
        </div>
          ...
          <Card>
            <CardHeader className="flex flex-row items-center justify-between">
              <div>
                ...
                <CardDescription>
                  {reportType === "adherence" &&
                    "Relatório de taxa de adesão por paciente"}
                  {reportType === "alerts" &&
                    "Relatório de alertas ignorados por paciente"}
                  {reportType === "complete" &&
              ...
              <div ref={tableRef}>
                        ...
                        filteredReportData.map((row) => (
                          <TableRow key={row.id}>
                            ...
                            <TableCell className="text-center">
                              <span
                                className={`px-2 py-1 rounded-md ${getAdesaoColor(
                                  row.adesao
                                )} ${getAdesaoTextColor(row.adesao)}`}
                              >
                                {row.adesao}%
                              </span>
                            </TableCell>
                ...
                {filteredReportData.length > 0 && (
                  <div className="mt-2 text-sm text-muted-foreground">
                    Mostrando {filteredReportData.length} de{" "}
                    {allReportData.length} pacientes
                  </div>
                )}
              </div>
            </CardContent>
          ...
          <div ref={summaryRef}>
            <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
              <Card>
                ...
                <CardContent>
                  <div className="text-2xl font-bold">
                    {statistics.adesaoMedia.toFixed(1)}%
                  </div>
              ...
              <Card>
                <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
                  <CardTitle className="text-sm font-medium">
                    Alertas Ignorados (Total)
                  </CardTitle>
                </CardHeader>
                <CardContent>
                  <div className="text-2xl font-bold">
                    {statistics.alertasTotal}
                  </div>
                  <p className="text-xs text-muted-foreground">
                    Média de{" "}
                    {(
                      statistics.alertasTotal / filteredReportData.length || 0
                    ).toFixed(1)}{" "}
                    por paciente
                  </p>
                </CardContent>
              ...
              <Card>
                ...
                <CardContent>
                  <div className="text-2xl font-bold">
                    {statistics.medicamentosMedia.toFixed(1)}
                  </div>
                  <p className="text-xs text-muted-foreground">
              ...
              <Card>
                ...
                <CardContent>
                  <div className="text-2xl font-bold">
                    {statistics.idadeMedia.toFixed(1)}
                  </div>
                  <p className="text-xs text-muted-foreground">
                    Faixa predominante: 60-75 anos
                  </p>
                </CardContent>
              </Card>
            </div>
            <Card className="mt-4">
              <CardHeader>
                ...
                <CardDescription>
                  Principais métricas e KPIs para análise
                </CardDescription>
              </CardHeader>
              <CardContent className="space-y-4">
                <div className="grid gap-4 md:grid-cols-2">
                  <div className="space-y-2">
                    ...
                    <div className="rounded-md border p-4">
                      {Object.entries(statistics.distribuicaoGenero).map(
                        ([genero, quantidade]) => (
                          <div
                            key={genero}
                            ...
                            <div className="flex items-center gap-2">
                              <div className="w-32 bg-gray-200 rounded-full h-2.5 dark:bg-gray-700">
                                <div
                                  ...
                                  style={{
                                    width: `${
                                      (quantidade / filteredReportData.length) *
                                      100
                                    }%`,
                                  }}
                                ></div>
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="rounded-md border p-4">
                      {Object.entries(statistics.distribuicaoRegiao).map(
                        ([regiao, quantidade]) => (
                          <div
                            key={regiao}
                            className="flex justify-between items-center mb-2"
                ...
                <div className="space-y-2">
                  ...
                  <ul className="list-disc pl-5 space-y-1">
                    <li>
                      Pacientes com mais de 3 medicamentos têm taxa de adesão
                      média de{" "}
                      {(
                        filteredReportData
                          .filter((p) => p.medicamentos > 3)
                          .reduce((acc, p) => acc + p.adesao, 0) /
                        (filteredReportData.filter((p) => p.medicamentos > 3)
                          .length || 1)
                      ).toFixed(1)}
                      %
                    </li>
                    <li>
                      Pacientes com menos de 3 medicamentos têm taxa de adesão
                      média de{" "}
                      {(
                        filteredReportData
                          .filter((p) => p.medicamentos < 3)
                    ...
                    <li>
                      A faixa etária acima de 70 anos apresenta maior número de
                      alertas ignorados
                    </li>
                    <li>
                      Horário com maior taxa de esquecimento:{" "}
                      {(() => {
                        const horarios = filteredReportData.flatMap(
                          (p) => p.horarioEsquecimento
                        );
                        const counts = horarios.reduce(
                          (acc: Record<string, number>, horario) => {
                            acc[horario] = (acc[horario] || 0) + 1;
                            return acc;
                          },
                          {}
                        );
                        const maxHorario = Object.entries(counts).sort(
                          (a, b) => b[1] - a[1]
                        )[0];
                        return maxHorario
                          ? `${maxHorario[0]} (${Math.round(
                              (maxHorario[1] / horarios.length) * 100
                            )}% dos casos)`
                          : "N/A";
                      })()}
                    </li>
